...
class LearningRateComparison(Scene):
    def construct(self):
        ...
        # Loss function: a simple parabola
        loss_func = lambda x: x**2 + 1

        # Plot the same loss curve on both
        small_curve = small_axes.plot(loss_func, color=BLUE)
        large_curve = large_axes.plot(loss_func, color=BLUE)

        # Small learning rate steps
        small_points = [0.9, 0.7, 0.5, 0.35, 0.2, 0.1, 0.05]
        small_dots = [Dot(small_axes.c2p(x, loss_func(x)), radius=0.06, color=WHITE) for x in small_points]
        small_arrows = [
            Arrow(
        ...
        # Large learning rate steps
        large_points = [-2.0, 2.0, -1.5, 1.5, 0.3]
        large_dots = [Dot(large_axes.c2p(x, loss_func(x)), radius=0.06, color=WHITE) for x in large_points]
        large_arrows = [
            Arrow(
        ...
        weight_label_l = Text("value of\nweight", font_size=20).next_to(small_axes.x_axis.get_end(), DOWN)
        weight_label_r = Text("value of\nweight", font_size=20).next_to(large_axes.x_axis.get_end(), DOWN)
